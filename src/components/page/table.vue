      ...
      <div v-if="show">
        ...
        <el-form :inline="true" :model="formData" class="demo-form-inline">
          ...
          <el-form-item>
            ...
            <el-button type="primary" size="small" @click="export2Excel">导出xls</el-button>
        ...
        <el-table ref="multipleTable" :data="
            tableData.length > 20
              ? tableData.slice(
                  page.pageSize * (page.pageNum - 1),
                  page.pageSize * page.pageNum
                )
              : tableData
          " border stripe height="630" style="width: 100%" v-loading="loading" @select="handleSelectionChange" @select-all="handleSelectionAll">
        ...
        <!-- 分页 -->
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="page.pageNum" :page-sizes="[5, 10, 15, 20]" :page-size="page.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="page.pageTotal"></el-pagination>
      </div>
    </transition>
    <!-- 编辑弹出框 -->
    ...
    Inquire() {
      ...
      if (searchVal) {
        this.tableData = this.tableData.filter(
          data =>
            !searchVal ||
            data.id.toLowerCase().includes(searchVal.toLowerCase())
        )
      } else {
